...
나머지 위치의 문자는 . 이거나 # 이다. . 은 빈 칸을 의미한다. 준식이는 빈 칸으로 자유롭게 이동할 수 있다. 
# 은 벽을 의미한다. 준식이는 벽이 있는 칸으로 이동할 수 없지만, 최대 M 개의 벽은 부술 수 있다.
...
# 준식이와 건틀렛 사이 함수 
def gauntlet_func(x,y):
    a = max(x,y)
    b = min(x,y)
    gauntlet_road = s[b+1:a]
    if gauntlet_road.count("#") > fight :
        return "HELP!"
    else :
        return "HAHA!"     
...
for _ in range(testcase):
    ...
    c = max(my_loca,exit_loca)
    d = min(my_loca,exit_loca)
    road = s[d+1:c]

    #준식이와 출구 사이 
    if road.count("#") > fight :
        ans.append(gauntlet_func(my_loca,gauntlet)) #준식이와 건틀렛 사이 
